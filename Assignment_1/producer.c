...
int main(int argc, char* argv[])
{
	...
	if(bufSize<0 || bufSize>1000){
		printf("Please enter bufSize between 0 and 1000\n");// Write code to check the validity of the command-line arguments001
	}
    InitShm(bufSize, itemCnt);// Function that creates a shared memory segment and initializes its header	
...
void InitShm(int bufSize, int itemCnt)
{
    int in = 0;
    int out = 0;
	const char *name = "OS_HW1_Harichandana_Karnam"; // Name of shared memory object to be passed to shm_open
...
void Producer(int bufSize, int itemCnt, int randSeed)
{
    ...
    for (i=1; i<=itemCnt; i++) {
        ...
        random_value=GetRand(0,3000);// Use the provided function GetRand() to generate a random number in the specified range
        in=GetIn();// Use the functions provided below to get/set the values of shared variables "in" and "out"
        printf("Producing Item %d with value %d at Index %d\n", i, random_value, in);// Use the following print statement to report the production of an item:
		// where i is the item number, val is the item value, in is its index in the bounded buffer
        WriteAtBufIndex(in,random_value);// Use the provided function WriteAtBufIndex() to write into the bounded buffer 	
        in=((in+1)%bufSize);
        SetIn(in);// **Extremely Important: Remember to set the value of any shared variable you change locally
}
